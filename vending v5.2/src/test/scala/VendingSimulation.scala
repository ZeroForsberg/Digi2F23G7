...
object VendingSimulation extends App {
  ...
  RawTester.test(new VendingMachine(20)) { dut =>
    ...
    while (d.running) {
      ...
      d.ledVal(14) = dut.io.led1.peek.litValue == 1
      d.ledVal(13) = dut.io.led2.peek.litValue == 1
      d.ledVal(12) = dut.io.led3.peek.litValue == 1
      d.ledVal(11) = dut.io.led4.peek.litValue == 1
      d.ledVal(0) = dut.io.alarm.peek.litValue == 1
      var price = 0
      for (i <- 0 until 6) {
        price <<= 1
        price += (if (d.switches(5-i).selected) 1 else 0)
      }
